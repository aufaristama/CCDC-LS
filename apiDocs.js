...
function Segments(segmentsImage, dateFormat, maxSegments) {
  return {
    /**
     * Create an intermediate object used to extract information about a date range.
     * 
    ...
    /**
     * Calculate value at the provided date, using as many harmonics as specified.
     * 
     ...
     * it is masked.
     * 
     * date: The date to calculate value for.
     * 
     * harmonics (default = 3): The number of harmonics to use from the model.
     *    Must be betweeen 0 and 3, where 0 only uses the linear components, and 3 uses all three harmonics.
     * 
     * Returns an ee.Image with the value and the intercept, slope, phase, amplitude, and RMSE for every calculated band.
     */ 
 ...
 *    0 = jDays, 1 = fractional years, 2 = unix time in milliseconds
 * 
 * defaultDate: Date to use when creating a slice from the segment and not specifying a date.
 */
function Segment(segmentImage, dateFormat, defaultDate) {
  return {
    /**
     * Takes a slice of the segment. It calculates the model value at a specified date, giving a synthetic image.
     * 
     * options: An object with the below keys.
     *    t (optional): The date to create the slice for, in the dateFormat used when constructing the Segment 
     *        as a number or ee.Image.
     * 
     *    date (default = defaultDate): The date to create the slice for, as an ee.Date, JavaScript Date or unix millis.
     *        Overrides t if both are specified.
     * 
     *    harmonics (default = 3): The number of harmonics to use when calculating the value.
     *        Must be betweeen 0 and 3, where 0 uses only the linear components, and 3 uses all three harmonics.
     * 
    ...
    /**
     * Take a slice at the start of the segment. It calculates the model value at the beginning of the segment, giving a synthetic image.
     * 
     *  harmonics (default = 3): The number of harmonics to use when calculating the value.
     *      Must be betweeen 0 and 3, where 0 uses only the linear components, and 3 uses all three harmonics.
     * 
     * Returns an ee.Image.
     */
    startSlice: function (harmonics) {},
    
    /**
     * Deprecated. Use startSlice() instead.
     */
    startFit: function (harmonics) {},
    
    /**
     * Take a slice at the middle of the segment. It calculates the model value at the middle of the segment, giving a synthetic image.
     * 
     *  harmonics (default = 3): The number of harmonics to use when calculating the value.
     *      Must be betweeen 0 and 3, where 0 uses only the linear components, and 3 uses all three harmonics.
     * 
     * Returns an ee.Image.
     */
    middleSlice: function (harmonics) {},
    
    /**
     * Deprecated. Use middleSlice() instead.
     */
    middleFit: function (harmonics) {},
    
    /**
     * Take a slice at the end of the segment. It calculates the model value at the end of the segment, giving a synthetic image.
     * 
     *  harmonics (default = 3): The number of harmonics to use when calculating the value.
     *      Must be betweeen 0 and 3, where 0 uses only the linear components, and 3 uses all three harmonics.
     * 
     * Returns an ee.Image.
     */
    endSlice: function (harmonics) {},
    
    /**
     * Deprecated. Use endSlice() instead.
     */
...
function DateRange() {
  return {    
    /**
     * Calculates the mean value for the date range.
     * 
     * harmonics (default = 3): The number of harmonics to use when calculating the mean.
     * 
     * Returns an ee.Image() with the mean of each base band.
     */
    mean: function (harmonics) {},    
    
    /**
     * Picks a breakpoint within the date range.
     * 
     * options: An object with the below keys.
     *    breakAnalysisBand (optional): The band to use when analyzing which breakpoint to provide data for.
     *    breakMagnitudeDirection (default = ANY): Specify which breakpoints to filter out, based on weather the break magnitude for the break analysis band decrease or increase.
     *        ANY - Include all breakpoints independent on their change magnitude direction.
     *        DECREASE - Only include breakpoints with a negative break magnitude in the break analysis band.
     *        INCREASE - Only include breakpoints with an positive break magnitude in the break analysis band.
     *    minBreakConfidence (default = 0): The minimum confidence of a breakpoint to be included. This is calculated by the break magnitude divided by the RMSE.
     *    breakSelection (default = FIRST): Specify which of the break points to provide data for.
     *        FIRST - Select the first breakpoint in the date range.
     *        LAST - Select the last breakpoint in the date range.
     *        CONFIDENCE - Select the breakpoint with the highest confidence.
     *        MAGNITUDE - Select the breakpoint with the highest break magnitude.
     *    
     * Returns an ee.Image() with tBreak and *_magnitude, and *_breakConfidence for each base band.
     */
